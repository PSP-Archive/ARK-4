...
{
    return; // ARK has no vshctrl module (for now)
	int newsize, result;
...
int InsertModuleBtcnfBefore(char* new_mod, char* before_mod, BtcnfHeader* header, int* size, u16 flags)
{
...
int _UnpackBootConfig(char **p_buffer, int length)
{
	...
	*/
    ...
    int res = InsertModuleBtcnfBefore("/kd/ark_systemctrl.prx", "/kd/init.prx", (void*)buffer, &newsize, 0x000000EF);
    if (res>=0) result = newsize;
    return result;
...
int SearchPrx(char* buffer, const char *modname)
{
	...
	for (int i=0; i<header->nmodules; i++){
	    if(strcmp(buffer + header->modnamestart + module[i].module_path, modname) == 0){
	        return i;
	    }
	}
	return -4;
	...
	*/
	...
	//iterate modules
	int modnum = 0;
	for(; modnum < header->nmodules; modnum++)
	{
		//found module name
...
int AddPRXNoCopyName(char * buffer, char * insertbefore, int prxname_offset, u32 flags)
{
	...
	memmove(&module[modnum + 1], &module[modnum], (u32)buffer + header->modnameend - (unsigned int)&module[modnum]);
	memcpy(&module[modnum], &newmod, sizeof(newmod));
	header->nmodules++;
	header->modnamestart += sizeof(newmod);
	header->modnameend += sizeof(newmod);
	//make mode include our module
	for(int modenum = 0; modenum < header->nmodes; modenum++)
	{
		//increase module range
		*(unsigned short *)((u32)buffer + header->modestart + modenum * 32) += 1;
	}
...
int AddPRX(char * buffer, char * insertbefore, char * prxname, u32 flags)
{
    ...
    int len = strlen(prxname)+1;
	header->modnameend += len;
	return AddPRXNoCopyName(buffer, insertbefore, header->modnameend - len - 2, flags);
}
...
{
	int modnum = SearchPrx(buffer, modname);
	if (modnum >= 0) {
	    _btcnf_header * header = (_btcnf_header *)buffer;
	    //cast module list
	    _btcnf_module * module = (_btcnf_module *)(buffer + header->modulestart);
