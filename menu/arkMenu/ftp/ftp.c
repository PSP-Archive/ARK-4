...
int mftpCreateDirIfNeeded(MftpConnection *con, char *Filename){
	SceIoStat fileStats;
...
int openDataConnectionPASV(MftpConnection *con){
	//mftpAddNewStatusMessage("Opening PASV PORT Ln - 100");
	...
	err = sceNetInetBind(con->sockPASV, (struct sockaddr*)&addrPort, sizeof(addrPort));
	if (err) return 0;
	...
	pasvPort++;
	//mftpAddNewStatusMessage("Okay Open Pasv Port! :D");
	return 0;
}

int openDataConnection(MftpConnection *con){
	//mftpAddNewStatusMessage("Try Open data port! ln - 131");
	int err;
	...
	if (con->usePassiveMode) {
		...
		con->sockData = sceNetInetAccept(con->sockPASV, (struct sockaddr*)&addrAccept, &cbAddrAccept);
		...
		err = sceNetInetConnect(con->sockData, (struct sockaddr*)&addrPort, sizeof(struct sockaddr_in));

		if (err){
			//mftpAddNewStatusMessage("Error Data port! :( ln 160;");
			return 0;
		}
	}
	//mftpAddNewStatusMessage("Data port Ok :D Ln - 164");
	return 1;
}
...
int mftpServerHello(MftpConnection *con){
	sendResponseLn(con, "220 ONElua Server ftp ready.");
	return 0;
}
...
int mftpCommandHELP(MftpConnection *con, char* command){
	...
	sendResponseLn(con, "214 Direct comments to onelua.x10.mx.");
...
int mftpCommandPASS(MftpConnection *con, char* command){
    ...
    else 
    {
        ...
        /*if (mftp_config.auth_required) { // Cualquier User es valido!
...
int mftpCommandPASV(MftpConnection *con, char* command){
	if (mftpRestrictedCommand(con, command)) {
		//mftpAddNewStatusMessage("ln 787 - Try Open Pasv Port! :P");
		char tmp[32];
...
int mftpRestrictedCommand(MftpConnection *con, char* command){ // Siempre Logeados :P
	/*if ((mftp_config.auth_required) && (!con->userLoggedIn)) { // Siempre estan logeados y no se requiere autentificacion
...
int mftpDispatch(MftpConnection *con, char* command){ // Acciones segun el comando!
  ...
  int renameFrom = 0;
	...
	if (strlen(uCommand)>0) {
      ...
      renameFrom = con->renameFrom;
		...
		} else if (strStartsWith(uCommand, "USER")) { // Revisar aqui quite el espacio despues de user, xD
			...
			sendResponse(con, command);
			sendResponseLn(con, " not understood.");
			//mftpAddNewStatusMessage("ln 951 - Comando desconocido...");
			//mftpAddNewStatusMessage(command);
			//pspDebugScreenPrintf("Command Unknow '500 ");
			//pspDebugScreenPrintf(command);
