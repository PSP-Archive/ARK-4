...
int TSRThread(SceSize args, void *argp) {
	...
	#ifdef DEBUG
	if (micro > 0) 
		scePaf_snprintf(vsh_menu.ark_version, sizeof(vsh->ark_version), "    ARK %d.%d.%.2i DEBUG    ", major, minor, micro);
	else 
		scePaf_snprintf(vsh_menu.ark_version, sizeof(vsh->ark_version), "    ARK %d.%d DEBUG    ", major, minor);
	#else
	if (micro > 0) 
		scePaf_snprintf(vsh_menu.ark_version, sizeof(vsh->ark_version), "    ARK %d.%d.%.2i    ", major, minor, micro);
	else 
		scePaf_snprintf(vsh_menu.ark_version, sizeof(vsh->ark_version), "    ARK %d.%d    ", major, minor); 
	#endif
