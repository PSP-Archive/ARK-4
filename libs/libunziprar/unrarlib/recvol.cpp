                         ...
                         const wchar *NameW,bool Silent)
{
  ...
  while (Find.Next(&RecData))
  {
    ...
    if (NewStyle)
    {
      ...
      {
#ifndef SILENT
        dummy_printf(St(MCRCFailed),Name);
#endif
        continue;
      }
    }
  ...
  {
    dummy_printf(St(MRecVolFound),FoundRecVolumes);
  }
#endif
  ...
  *LastVolName=0;
  ...
  for (int CurArcNum=0;CurArcNum<FileNumber;CurArcNum++)
  {
    ...
    if (ValidVolume)
    {
        ...
        while (!EndFound && NewFile->ReadHeader()!=0)
        {
          ...
          if (NewFile->GetHeaderType()==ENDARC_HEAD)
          {
                ...
                NewFile->EndArcHead.ArcDataCRC!=CalcFileCRC(NewFile,NewFile->CurBlockPos))
            {
              ValidVolume=false;
#ifndef SILENT
              dummy_printf(St(MCRCFailed),ArcName);
#endif
            }
            EndFound=true;
          }
      ...
      if (!ValidVolume)
      {
        ...
        strcat(NewName,".bad");
#ifndef SILENT
        dummy_printf(St(MBadArc),ArcName);
        dummy_printf(St(MRenaming),ArcName,NewName);
#endif
...
#ifndef SILENT
      dummy_printf(St(MAbsNextVol),ArcName);
#endif
    }
...
#ifndef SILENT
  dummy_printf(St(MRecVolMissing),MissingVolumes);
#endif

  if (MissingVolumes==0)
  {
#ifndef SILENT
    dummy_printf(St(MRecVolAllExist));
#endif
    return(false);
  }
  ...
  {
#ifndef SILENT
    dummy_printf(St(MRecVolCannotFix));
#endif
    return(false);
  }
#ifndef SILENT
  dummy_printf(St(MReconstructing));
#endif
  ...
  while (true)
  {
    ...
    {
      dummy_printf("\b\b\b\b%3d%%",CurPercent);
