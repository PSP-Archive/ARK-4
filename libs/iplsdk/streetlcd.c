...
static StreetLcdState *lcd_state(void)
{
    static StreetLcdState s_lcd_state;
    return &s_lcd_state;
}

static void enable_display(void)
{
    gpio_set(GPIO_PORT_DEBUG_1);
}

static void disable_display(void)
{
    gpio_clear(GPIO_PORT_DEBUG_1);
}
...
static void update_display_state(StreetLcdState *state)
{
...
static void update_brightness(StreetLcdState *state, int brightness)
{
    // rescale to 0 -> 25
...
static LcdDriverResult lcd_set_mode(int mode, unsigned int xres, unsigned int yres, unsigned int stride, unsigned int format)
{
    return LCD_DRIVER_ERR_NOT_IMPL;
}

static LcdDriverResult lcd_set_backlight_brightness(unsigned int brightness)
{
    lcd_state()->requested_brightness = brightness;
    return LCD_DRIVER_RES_OK;
}

static LcdDriverResult lcd_enable_display(void)
{
    lcd_state()->requested_display_state = DISPLAY_ON;
    return LCD_DRIVER_RES_OK;
}

static LcdDriverResult lcd_disable_display(void)
{
    lcd_state()->requested_display_state = DISPLAY_OFF;
    return LCD_DRIVER_RES_OK;
}

static LcdDriverResult lcd_on_interrupt(void)
{
    StreetLcdState *state = lcd_state();
...
static LcdDriverResult lcd_init(void)
{
    StreetLcdState *state = lcd_state();
    ...
    gpio_set_port_mode(GPIO_PORT_DEBUG_1, GPIO_MODE_OUTPUT);
    return LCD_DRIVER_RES_OK;
}

const LcdDriver g_street_lcd_driver = 
{
    .init = lcd_init,
